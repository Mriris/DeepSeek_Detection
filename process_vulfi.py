...
def extract_vulfi_data(vulfi_data, extracted_functions):
            ...
            for instruction in function['instructions']:
                ...
                if address:
                    address_to_instruction[address] = {
                        ...
                        'function_name': function.get('function_name', '未知函数')
                    }
    ...
    for vulfi_item in vulfi_data:
        ...
        if address in address_to_instruction:
            ...
            existing_entry = next((entry for entry in vulfi_extracted_data if entry['function_name'] == function_name), None)
            if existing_entry:
...
def priority_to_numeric(priority):
    priority_map = {'High': 3, 'Medium': 2, 'Low': 1, '': 0, None: 0}
