...
class VulFiScanner:
    def __init__(self, custom_rules=None):
        ...
        with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "vulfi_prototypes.json"),
                  "r") as proto_file:
        ...
        for rule in self.rules:
            ...
            for scanned_function in xrefs_dict:
                ...
                for scanned_function_xref_tuple in xrefs_dict[scanned_function]:
                    ...
                    if rule["function_names"] == ["Array Access"]:
                        param = [VulFiScanner.Param(self, scanned_function_xref_tuple[3], scanned_function_xref,
                                                    scanned_function_name),
                                 VulFiScanner.Param(self, scanned_function_xref_tuple[4], scanned_function_xref,
                                                    scanned_function_name)]
                    elif rule["function_names"] == ["Loop Check"]:
                        param = [VulFiScanner.Param(self, scanned_function_xref_tuple[3], scanned_function_xref,
                                                    scanned_function_name),
                    ...
                    try:
                        if not param:
                            priority = "Info"
                        elif eval(rule["mark_if"]["High"],
                                  dict(self=self, param=param, param_count=param_count, function_call=function_call)):
                            priority = "High"
                        elif eval(rule["mark_if"]["Medium"],
                                  dict(self=self, param=param, param_count=param_count, function_call=function_call)):
                            priority = "Medium"
                        elif eval(rule["mark_if"]["Low"],
                                  dict(self=self, param=param, param_count=param_count, function_call=function_call)):
                            priority = "Low"
                    ...
                    if priority and not "wrapped" in scanned_function_display_name:
                        results.append(list(
                            VulFi.result_window_row(rule["name"], scanned_function_display_name, found_in_name,
                                                    hex(scanned_function_xref), "Not Checked", priority, "")))
                        ...
                        skip_count = int(
                            scanned_function_display_name[scanned_function_display_name.find("wrapped:") + 8:-1])
        ida_kernwin.hide_wait_box()
        return results
                ...
                try:
                    ...
                    for tree_item in decompiled_function.treeitems:
                        if tree_item.op == ida_hexrays.cit_while:
                            ...
                            if (op >= ida_hexrays.cot_land and op <= ida_hexrays.cot_ult) or op in [ida_hexrays.cot_var,
                                                                                                    ida_hexrays.cot_lnot]:
                                # Not an infinte loop
                                counter, check_val = self.parse_condition(tree_item.to_specific_type.cwhile.expr)
                                loop_check_list["Loop Check"].append(
                                    (tree_item.ea, "Loop Check", "Loop Check", counter, check_val))
                            else:
                                counter, check_val = self.get_loop_check_val_break(
                                    tree_item.to_specific_type.cwhile.body)
                                loop_check_list["Loop Check"].append(
                                    (tree_item.ea, "Loop Check", "Loop Check", counter, check_val))
                            ...
                            if tree_item.to_specific_type.cfor.expr.op == ida_hexrays.cot_empty:
                                ...
                                loop_check_list["Loop Check"].append((tree_item.ea, "Loop Check", "Loop Check", counter,
                                                                      self.get_loop_check_val(counter,
                                                                                              tree_item.to_specific_type.cfor.expr)))
                ...
                try:
                    ...
                    for citem in decompiled_function.treeitems:
                        if citem.op == ida_hexrays.cot_idx:
                            ...
                            array_access_list["Array Access"].append((citem.ea, "Array Access", "Array Access",
                                                                      citem.to_specific_type.x,
                                                                      citem.to_specific_type.y))
                except Exception as e:
                    pass
        return array_access_list
            ...
            for xref in idautils.XrefsTo(function_address):
                ...
                if ida_ua.decode_insn(insn, xref.frm) != idc.BADADDR:
                    if (insn.get_canon_feature() & 0x2 == 0x2 or  # The instruction is a call
                            (insn.get_canon_feature() & 0xfff == 0x100 and insn.Op1.type in [idc.o_near,
                                                                                             idc.o_far])):  # The instruction uses first operand which is of type near/far immediate address
                        ...
                        if wrappers:
                            ...
                            if not retrieved_wrappers:
                                ...
                                # Wrappers were found
                                wrapped_tupple = (
                                xref_tuple[0], xref_tuple[1], f"{xref_tuple[2]} (wrapped:{len(retrieved_wrappers)})")
                                xrefs_list[current_function_name].append(
                                    wrapped_tupple)  # this makes sure that the wrapped function is right before its wrappers
                    ...
                    if xref_func_name == current_function_name.lower():
                        ...
                        if found:
                            for wrapper_xref in idautils.XrefsTo(decompiled_function.entry_ea):
                                ...
                                wrapper_xrefs.append((wrapper_xref.frm, wrapper_xref_func_name,
                                                      f"{wrapper_xref_func_name} ({current_function_name} wrapper)"))
        return wrapper_xrefs
            ...
            if self.scanner_instance.hexrays:  # hexrays
                ...
                # If it is a cast (could happen)
                    ...
                    if expr.x.obj_ea != idc.BADADDR:
                        c_str_pointer_value = idc.get_bytes(expr.x.obj_ea + 2 * self.scanner_instance.ptr_size,
                                                            self.scanner_instance.ptr_size)
                        tmp_ea = int.from_bytes(c_str_pointer_value, byteorder=self.scanner_instance.endian)
                        c_str_len = int.from_bytes(idc.get_bytes(expr.x.obj_ea + 3 * self.scanner_instance.ptr_size,
                                                                 self.scanner_instance.ptr_size),
                                                   byteorder=self.scanner_instance.endian)
                ...
                if expr.type == 0x2:
                    ...
                    if idc.get_strlit_contents(expr.addr):
                        ...
                        addr = int.from_bytes(idc.get_bytes(expr.addr, self.scanner_instance.ptr_size),
                                              byteorder=self.scanner_instance.endian)
    ...
    class FunctionCall:
        ...
        def return_value_checked_disass(self, check_val):
            ...
            for basic_block in idaapi.FlowChart(idaapi.get_func(self.call_xref)):
                if self.call_xref >= basic_block.start_ea and self.call_xref < basic_block.end_ea and len(
                        list(basic_block.succs())) > 1:
                    ...
                    if xref_func_name == self.scanned_function.lower():
                        ...
                        if (parent.op >= ida_hexrays.cot_eq and parent.op <= ida_hexrays.cot_ult):
                            if check_val is not None:
                                ...
                                if parent.y and (
                                        parent.y.n or parent.y.fpc):  # Check if there is a Y operand and if it is a number, if it is, get its value
                                    if parent.y.n:  # int
                            ...
                            if parent.to_specific_type.x.v or parent.to_specific_type.x.op == ida_hexrays.cot_obj:
                                for sub_tree_item in list(decompiled_function.treeitems)[index:]:
                                    if sub_tree_item.to_specific_type.op >= 22 and sub_tree_item.to_specific_type.op <= 31:
                                        # Comparison operator
                                        if (
                                                sub_tree_item.to_specific_type.x.v and sub_tree_item.to_specific_type.x.v.idx == parent.to_specific_type.x.v.idx) or (
                                                sub_tree_item.to_specific_type.x.obj_ea and sub_tree_item.to_specific_type.x.obj_ea == parent.to_specific_type.x.obj_ea):
                                            if check_val is not None:
                                                numeric_val = sub_tree_item.to_specific_type.y
                                                if numeric_val and (
                                                        numeric_val.n or numeric_val.fpc):  # Check if there is a Y operand and if it is a number, if it is, get its value
                                            ...
                                            while True:
                                                if embedded_parent.op == ida_hexrays.cit_if:
                                                    ...
                                                    embedded_parent = decompiled_function.body.find_parent_of(
                                                        embedded_parent)
...
class VulFi_Single_Function(idaapi.action_handler_t):
    ...
    def activate(self, ctx):
        ...
        if not idc.get_type(self.function_ea) and (
                idaapi.get_func(self.function_ea) and not idc.get_type(idaapi.get_func(self.function_ea).start_ea)):
            # If the type of the function is not set notify the user
            answer = ida_kernwin.ask_buttons("Yes", "No", "Cancel", 1,
                                             f"You should first set type for the function. Continue without type anyway?")
            if answer != 1:
                return
        ...
        tmp_rule = [
            {"name": f"{custom_rule_name}", "function_names": [function_name.lower(), f".{function_name.lower()}"],
             "wrappers": False, "mark_if": {"High": custom_rule, "Medium": "False", "Low": "False"}}]
        ...
        for item in vulfi_data:
            rows.append([vulfi_data[item]["name"], vulfi_data[item]["function"], vulfi_data[item]["in"],
                         vulfi_data[item]["addr"], vulfi_data[item]["status"], vulfi_data[item]["priority"],
                         vulfi_data[item]["comment"]])
        ...
        # Save the results
        for item in rows:
            vulfi_data[f"{item[3]}_{item[0]}"] = {"name": item[0], "function": item[1], "in": item[2], "addr": item[3],
                                                  "status": item[4], "priority": item[5], "comment": item[6]}
        ...
        results_window.AddCommand("Mark as False Positive", flags=4, menu_index=-1, icon=icon_id, emb=None,
                                  shortcut=None)
...
class VulFi(idaapi.action_handler_t):
    ...
    def activate(self, ctx):
        ...
        # Always use default scan
        print("[VulFi] Using default rules for scan.")
        ...
        # Run the scan automatically without asking
        print("[VulFi] Started the scan ...")
        scan_result = vulfi_scanner.start_scan(marked_addrs)
        if scan_result is None:
            return
        ...
        # Construct and show the form (optional if you still want the results window)
        results_window = VulFiEmbeddedChooser(self.result_window_title, self.result_window_columns, rows, icon_id)
        results_window.Show()

        # Directly call vulfi_export to automatically export the results after scanning is done
        results_window.vulfi_export()
...
class VulFiEmbeddedChooser(ida_kernwin.Choose):
    ...
    def save(self):
        ...
        vulfi_dict = {}
        for item in self.items:
            vulfi_dict[f"{item[3]}_{item[0]}"] = {"name": item[0], "function": item[1], "in": item[2], "addr": item[3],
                                                  "status": item[4], "priority": item[5], "comment": item[6]}
        node = idaapi.netnode()
        node.create("vulfi_data")
        # Set the blob
...
class vulfi_fetch_t(idaapi.plugin_t):
    ...
    def init(self):
        vulfi_desc = idaapi.action_desc_t(
            ...
            'Make VulFi fetch the potentially interesting places in binary.',
    ...
    # def init(self):
    #     print("[VulFi] Plugin initialized.")
    #     return idaapi.PLUGIN_OK
...
class Hooks(idaapi.UI_Hooks):
    ...
    def finish_populating_widget_popup(self, form, popup):
        ...
        action_desc = idaapi.action_desc_t(
            ...
            '',  # Optional: the action shortcut
            'Make VulFi look for all interesting refences of this function.',
            # Optional: the action tooltip (available in menus/toolbar)
            icon_id)  # Optional: the action icon (shows when in menus/toolbars)
        idaapi.unregister_action("vulfi:get_one")
        idaapi.register_action(action_desc)
        if ida_kernwin.get_widget_type(form) == idaapi.BWN_DISASM or ida_kernwin.get_widget_type(
                form) == idaapi.BWN_PSEUDOCODE:
